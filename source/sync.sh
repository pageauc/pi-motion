...
# --------------------------------------------------------------------
# Requires /usr/local/rclone executable 
...
WATCH_APP='pi-motion.py'     # App filename to monitor for Run Status
...
CHECK_FOR_SYNC_FILE = $CHECK_FOR_SYNC_FILE
SYNC_FILE_PATH      = $SYNC_FILE_PATH

REMOTE_CONFIG_ON    = $REMOTE_CONFIG_ON
REMOTE_CONFIG_DIR   = $REMOTE_CONFIG_DIR
REMOTE_CONFIG_FILE  = $REMOTE_CONFIG_FILE
LOCAL_CONFIG_FILE   = $LOCAL_CONFIG_FILE

REMOTE_WIPE_ON      = $REMOTE_WIPE_ON
REMOTE_WIPE_DIR     = $REMOTE_WIPE_DIR
REMOTE_WIPE_FILE    = $REMOTE_WIPE_FILE
REMOTE_WIPE_SAFE    = $REMOTE_WIPE_SAFE

WATCH_APP_ON        = $WATCH_APP_ON
WATCH_APP           = $WATCH_APP
FORCE_REBOOT        = $FORCE_REBOOT
==================================================================
STATUS  - Current pi-timolo working dir is $PROG_DIR
START   - Processing"
...
function do_rclone_sync ()
{
    ...
    echo "------------------------------------------"
    /usr/bin/rclone sync file-id $SYNC_DIR
    if [ ! $? -eq 0 ] ; then
        echo "------------------------------------------"
        echo "STATUS  - Creating Remote Folder /$SYNC_DIR"
        echo "------------------------------------------"
        /usr/bin/rclone new --folder $SYNC_DIR
        /usr/bin/rclone file-id $SYNC_DIR
        if [ $? -eq 0 ] ; then
            echo "------------------------------------------"
    ...
    echo "
    ------------------------------------------
    STATUS  - Start gdrive Sync ....
    STATUS  - Local Source Folder - $PROG_DIR/$SYNC_DIR
    STATUS  - Remote Destn Folder - /$SYNC_DIR
    STATUS  - Files $FILES_TO_SYNC
    STATUS  - Running  This May Take Some Time .....
    STATUS  - /usr/bin/rclone sync $SOURCE_DIR $REMOTE:$/DEST_DIR
    ------------------------------------------
    "
    date
    START=$(date +%s)
    /usr/bin/rclone sync $SYNC_SOURCE_DIR $REMOTE:$SYNC_DEST_DIR
    if [ $? -ne 0 ] ;  then
        # Check if rclone sync was successfully
        date
        echo "------------------------------------------"
        echo "ERROR   - rclone Sync Failed."
        echo "          Possible Cause - See rclone Error Message Above"
